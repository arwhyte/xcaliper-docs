...
A 2016 IMS [comparison](https://www.imsglobal.org/initial-xapicaliper-comparison) of Caliper and xAPI described the two specifications as not equivalent but complementary.  We detect a good deal of overlap between the two specifications.  That said, we believe that Caliper's design, governance, and increasingly, industry support provides a firmer base than xAPI on which to build an interoperable and scalable learning data services infrastructure.  We recommend that Caliper/xAPI "convergence" take the form of an xAPI-to-Caliper migration strategy (codename *xCaliper*) developed by IMS in concert with ADL.  Caliper would be positioned as a superset of xAPI.  The Caliper information model would be adjusted to capture certain xAPI concepts and attributes not currently described.  An xCaliper transducer service for converting xAPI statements to Caliper Events would be developed as an aid to migration (outlined below).  xCaliper should also encompass the creation of an IMS community-supported "Profile" repository for publicizing and sharing Caliper and xAPI profile and recipe assets.
