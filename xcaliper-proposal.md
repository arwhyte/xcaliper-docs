...
A 2016 IMS [comparison](https://www.imsglobal.org/initial-xapicaliper-comparison) of Caliper and xAPI described the two specifications as not equivalent but complementary.  We concur with this view.  We believe addressing .edu concerns should be Caliper's primary focus.  We also believe that Caliper's design, governance, and increasingly, industry support is well suited to support an interoperable and scalable learning data services infrastructure for the .edu domain.  With that in mind we propose *xCaliper* as a program that aims to both promote .edu Caliper adoption as well as support IMS members and others who need to support both Caliper and xAPI.  We propose the folowing xCaliper initiatives:
...
* Caliper is not dependent on other IMS specifications. Nevertheless it benefits from opportunities to align with sibling specifications such as Learning Tools Interoperability (LTI) and OneRoster.  xAPI currently neither enforces vocabulary agreement between adopters nor benefits from the synergies implicit in the IMS ecosystem. 
